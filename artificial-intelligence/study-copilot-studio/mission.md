...
**\~20 sessions** to working proficiency, recomputed group by group with a standing Session-Sizing Engine pass before each group starts. Group 2 added one spaced-review session over its original plan (2.4); Group 3 added one grounding-and-citations session over its original plan (3.2). Group 4 was sized before 4.1 started at 5 sessions, up from the original 4 — connectors (4.1), agent flows (4.2), custom prompts/REST APIs (4.3), and adaptive cards (4.4) each carried their own distinct integration mechanism that didn't compress safely into a shared session, and a closing review/build session (4.5) was added to match the same spaced-review pattern 2.4 and Group 3 both established.

**Group 4 is now complete.** 4.5 closed it out by wiring 4.3's REST API tool, 4.2's human-review flow, and 4.4's adaptive-card completion into one Northwind return-and-refund build — the same consolidation shape 2.4 and 3.3 both used to close their own groups. The running estimate holds at **\~25–27 sessions**, recomputed one group at a time. **A Group 5 sizing pass is next, before 5.1 starts** — the standing rule that already fired identically before Groups 2, 3, and 4.
...
| 5 — Agentic Behavior: Autonomous Agents (sizing pass next) | 5.1 | Designing autonomous agent capabilities                      | ⬜ Next   |
